...
🎯 **상태**: 진행 중 | 📅 **추정 완료**: 4-6주 (1명 기준) | 🔥 **우선순위**: Phase 2 구현 완료 → Phase 3 Firebase 연동

## 📚 주요 문서

### 🎯 현황 및 진행상태

> **[`PROJECT_STATUS_SUMMARY.md`](./PROJECT_STATUS_SUMMARY.md)** — **전체 프로젝트 진행 현황** 
> 모듈별 완성도, 남은 작업, 시간 투자 분석, 우선순위별 로드맵, Day 0 런칭 준비 현황 (60% 완료)
...
- **[`PHOTOMANAGEMENTPAGE_DESIGN.md`](./PHOTOMANAGEMENTPAGE_DESIGN.md)** - PhotoManagementPage 상세 설계 (Project 구조, UI 레이아웃, CRUD, 사진 관리)
...
| | CreateNewOrderPage | `/orders/new` | ✅ 완료 | 새 주문 생성 (Remote Config 가격, Firestore 생성) |
| | OrderDetailsPage | `/orders/:orderId` | ✅ 완료 | 주문 상세 (onSnapshot, 타임아웃 카운트다운) |
| | PaymentPage | `/orders/:orderId/payment` | ✅ 완료 | 결제 처리 (진입조건 검증, MockPayment) |
| **사진관리** | PhotoManagementPage | `/photo-management` | ✅ 완료 | 사진 관리 (onSnapshot, Storage 업로드, 7가지 상태) |
...
> **상세 스펙**: [`MASTER_SPEC.md`](./MASTER_SPEC.md) 참조 (구현 체크리스트 포함)

**구현 완료 항목:**
- [x] PhotoManagementPage — Firestore onSnapshot, Cloud Storage 업로드, 7가지 사진 상태 머신, 선택 → 주문 생성 플로우
- [x] CreateNewOrderPage — sessionStorage 복원, Remote Config 기반 실시간 가격 계산, Firestore `orders/` 문서 생성 (priceSnapshot)
- [x] OrderDetailsPage — Firestore onSnapshot 실시간 구독, 타임아웃 카운트다운 (clearInterval cleanup 포함)
- [x] PaymentPage — 진입 조건 검증 (1시간 이내), 결제 방법 선택, MockPayment + `Order.status = PAID`
- [x] PriceConfigService — Firebase Remote Config + 로컬 기본값 fallback
- [x] PhotoService / StorageService — 사진 상태 관리 및 Cloud Storage 업로드
- [x] Cloud Function: `processUploadedPhoto` — 썸네일/WebP/백업 생성 (sharp), READY 상태 업데이트

**남은 작업 (Phase 3+):**
- [ ] TODO: Admin Dashboard (고아 파일 모니터링)
- [ ] TODO: Cloud Scheduler (자동 정리 Cron Job)
- [ ] TODO: 실제 PG 연동 (포트원 / 토스페이먼츠)
...
#### 주문 관리 (Phase 2) — ✅ 완료
- ✅ PhotoManagementPage — Dropbox 스타일 UI, 7가지 사진 상태 머신, onSnapshot 실시간 리스너
- ✅ CreateNewOrderPage — Remote Config 가격 계산, Firestore 주문 생성
- ✅ OrderDetailsPage — onSnapshot 구독, 타임아웃 카운트다운
- ✅ PaymentPage — 진입 조건 검증, MockPayment, Order.status = PAID
- ✅ PriceConfigService — Remote Config + 로컬 기본값
- ✅ PhotoService / StorageService — 사진 CRUD, Storage 업로드
- ✅ Cloud Function: processUploadedPhoto — sharp 이미지 처리, READY 업데이트

### 🔄 진행 중인 기능

- 🔄 Cloud Storage 보정 파일 다운로드 연동
- 🔄 Phase 3 샘플보정 Firebase 실제 연동
- 🔄 실시간 상태 업데이트 (폴링 → onSnapshot 전환)

### ⚪ 대기 중인 기능

- ⚪ 실제 PG 연동 (포트원 / 토스페이먼츠)
- ⚪ Cloud Scheduler (자동 정리 Cron Job)
- ⚪ 설정 페이지 (프로필, 알림 설정)
- ⚪ 알림 센터 (메일, In-App 알림)
- ⚪ 1:1 문의하기
